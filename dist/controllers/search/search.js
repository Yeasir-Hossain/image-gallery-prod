...
var _orama = require("@orama/orama");
var _pluginDataPersistence = require("@orama/plugin-data-persistence");
...
class SearchCtrl {
  constructor() {
    ...
    */
     ...
     * @type {string}
    */
    this.dataPath = _path.default.join(_path.default.resolve(), '.cache');
    /**
  ...
  */
  save(keys = Object.keys(this.schemas)) {
    return Promise.all(keys.map((k, i) => (0, _pluginDataPersistence.persistToFile)(this.instances[k], 'binary', `${_path.default.join(this.dataPath, keys[i] + '.msp')}`)));
  }
  ...
  */
  async start() {
    ...
    await Promise.all(keys.map(async k => this.instances[k] = (await this.restore(`${_path.default.join(this.dataPath, k + '.msp')}`)) || (await (0, _orama.create)({
  ...
  */
  async insert(key, value) {
    return await (0, _orama.insert)(this.instances[key], value);
  }
  ...
  */
  async remove(key, id) {
    try {
      await (0, _orama.remove)(this.instances[key], id);
    } catch (e) {
      //
    }
  }
  ...
  */
  async search(key, query) {
    return await (0, _orama.search)(this.instances[key], _objectSpread(_objectSpread({}, query), {}, {
      term: query.term || '',
